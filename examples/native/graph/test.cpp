...
std::string parse_dot(const std::string& dot, int w, int h)
{
    static int dpi = 100;
    ...
    agsafeset( G, "size", (char*)(oss.str().c_str()), "8,4!");
    ...
    gvRenderData( gvc, G, "png", &buffer, &bufsize);


/*
    std::ofstream of;
    of.open("test.png", std::ios::binary);
    if(of)
    {
...
*/
    ...
    std::string ret = "data:image/png;base64,";
    ret += cryptoneat::Base64::encode(buffer,bufsize);
...
*/
        send_request( web, "get_size" ).then( [this]( ResultFuture<Json::Value> result )
        {
            Json::Value size = result.get();
            gstring dot = mtk_editor_get_text(editor);

            int w = size["width"].asInt();
            int h = size["height"].asInt();

            std::string png = parse_dot(dot.str(), w, h);
            if(png.empty())
            {
                return;
            }
            send_request( web, "setImage", png );
